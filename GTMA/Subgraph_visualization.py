...
def Whole_supernode_vis(WSI, Position_pd, Location_pd, Select_ID, core_dir, edge_mask_norm, row, col):
    ...
    print(os.path.join(core_dir, Select_ID + '_WSI_graph_wo_IG.jpeg'))
    plt.clf()
    plt.cla()
    plt.close()
...
def Calculate_IG(d, d_count, row, col, edge_mask, edge_mask_norm, patient_target_dir,
    ...
    dataset_root2 = '/mnt/usb/huamenglei/GC_2300/GC/original/'
    p_id = d.absolute_path[d_count].split('_')[0]
    id_path = d.absolute_path[d_count]

    WSI_dir = os.path.join('/mnt/usb3/1.WSI_数据/1.GC_2300/WSI数据')
    ...
    Location_pd = pd.read_csv(
        dataset_root2 + '_'.join(id_path.split('_')[0:1]) + '_node_location_list.csv')
...
def Subgraph_analysis_vis(Argument):
    ...
    Metadata = pd.read_csv('/home/stat-huamenglei/TEA-graph-master/GC_2300_all.csv')
    ...
    Trainlist = [item for c, item in enumerate(Trainlist) if
             (("_0.75_graph_torch_4.3_artifact_sophis_final.pt" in item))]
