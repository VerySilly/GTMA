...
def Vis_main(Argument):
    # Argument.load_state_dict 根据fold要进行改变
    # Fi = Argument.FF_number 或者 直接修改Fi
    Fi = int(Argument.load_state_dict.split('/')[-2][0])
    Argument.FF_number = Fi
    ...
    Trainlist = [item for c, item in enumerate(Trainlist) if
             (("0.75_graph_torch_4.3_artifact_sophis_final.pt" in item))]
