...
def Analyze(Argument, best_model, checkpoint_dir, Figure_dir, bestepoch, best_select=True):
    ...
    Metadata = pd.read_csv('/home/stat-huamenglei/数据合集/TCGA_GC临床信息/clinical.tsv', sep='\t')
    

    TrainRoot = TrainValid_path(Argument.DatasetType) 
    Trainlist = os.listdir(TrainRoot)

    csv_id = list(pd.read_csv('/home/stat-huamenglei/271ids_clinical_days_30.csv')['case_submitter_id'])
    Trainlist = [id for id in Trainlist if id[:12] in csv_id]
    print(len(Trainlist))
    # Trainlist = [item for c, item in enumerate(Trainlist) if '0_graph_torch_4.3_artifact_sophis_final.pt' in item]
    # 原代码取前100个Trainlist = Trainlist[:100]
