...
def metadata_list_generation(DatasetType, Trainlist, Metadata):
    ...
    if "BORAME" in DatasetType:
        ...
        # Trainlist2 = Trainlist.copy()
        with tqdm(total=len(Trainlist)) as tbar:
            for idx in range(len(Trainlist)):
                # Trainlist 是wsi的地址 但是仅为了获得id
                item = int(Trainlist[idx].split('/')[-1].split('.pt')[0].split('_')[0][:7])
                Match_item = Metadata[Metadata["WSIs"] == item]
                if Match_item.shape[0] != 0:
                    if Match_item['death_status'].tolist()[0] == 0:
                        Train_censorlist.append(0)
                        Train_survivallist.append(int(float(Match_item['death_time'].tolist()[0])))
                        if 'Ⅳ' in Match_item['ajcc_pathologic_stage'].tolist()[0]:
                            Train_stagelist.append(4)
                        elif "Ⅲ" in Match_item['ajcc_pathologic_stage'].tolist()[0]:
                            Train_stagelist.append(3)
                        elif "Ⅱ" in Match_item['ajcc_pathologic_stage'].tolist()[0]:
                            Train_stagelist.append(2)
                        elif "Ⅰ" in Match_item['ajcc_pathologic_stage'].tolist()[0]:
                            Train_stagelist.append(1)
                        else:
                            Train_stagelist.append(0)
                    else:
...
def makecheckpoint_dir_graph(Argument):
    ...
    checkpoint_dir = os.path.join('/mnt/usb4/hml/TEA_graph/results/', Argument.DatasetType, Argument.model)
    checkpoint_dir = os.path.join(checkpoint_dir, str(Argument.FF_number)+'FOLD')
...
class coxph_loss(torch.nn.Module):
    ...
    def forward(self, risk, phase, censors):
        ...
        loss = -torch.sum(censored_likelihood) / float(censors.nonzero(as_tuple=False).size(0))
...
def TrainValid_path(DatasetType):
    ...
    if DatasetType == "BORAME":
        ...
        Pretrain_root = "/mnt/usb4/hml/TEA-graph-master/data/Graph_test_resize/TCGA/GC/superpatch"
    elif DatasetType == "NLST":
        Pretrain_root = "./Sample_data_for_demo/Graphdata/NLST/"
    else:
        Pretrain_root = "/mnt/usb/huamenglei/GC_2300/GC/superpatch"
